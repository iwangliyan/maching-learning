...
#    X_train = each[each.datetime < datetime.datetime(2017,11,1,0,0,0)].iloc[:,5:]
#    y_train = each[each.datetime < datetime.datetime(2017,11,1,0,0,0)].iloc[:,1]
...
for i in range(1,34):
    for j in range(i+1,34):
        dfi = df[df.loc_id==i]
        dfj = df[df.loc_id==j]
        dff = pd.merge(dfi,dfj,on='datetime',how='inner')
        corr = dff.phone_id_x.corr(dff.phone_id_y,'spearman')
        if corr >= 0.9:
            print('i:',i,'j:',j,'corr:',corr)