...
train_full.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/train_9_10完整时间人流量_时间分解.csv', index=False)
'''
...
for i in range(1, 8):  # 指明周一到周日
    ...
    for j in range(0, 24):  # 指明24个小时
        ...
        zhongwei = np.zeros((4, 33), dtype=float)
        ...
        for k in range(3, 7):  # 指明周数
            ...
            if np.sum(temp) != 0:
                for m in range(33):  # 记录每小时的中位数占比
                    zhongwei[k-3][m] = (weekday_hour_week.iloc[m, 2])/np.sum(temp)  # 记录每小时的中位数占比
...
result_location_hour.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/9_10规则三_4周.csv', index=False)