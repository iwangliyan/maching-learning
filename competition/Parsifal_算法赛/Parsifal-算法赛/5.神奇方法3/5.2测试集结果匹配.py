...
# 读取数据
path_test_half = 'E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/9_10规则三_6周_人群分类.csv'
path_test_time = 'E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/test完整时间序列.csv'
test_half = pd.read_csv(path_test_half)
test_time = pd.read_csv(path_test_time)

# 添加共有时间信息，做key进行匹配
test_half.columns = ['weekday1', 'hour1', 'location1', 'flow']
...
print(test_time.head(3))
test_half['Location+weekday+hour'] = test_half['location1'].map(str) + '-' + test_half['weekday1'].map(str) + '-' + test_half['hour1'].map(str)
test_time['Location+weekday+hour'] = test_time['location'].map(str) + '-' + test_time['weekday'].map(str) + '-' + test_time['hour'].map(str)
print(test_half.head(3))
print(test_time.head(3))
...
test_full.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/规则三_全时间6_人群分类.csv', index=False)
