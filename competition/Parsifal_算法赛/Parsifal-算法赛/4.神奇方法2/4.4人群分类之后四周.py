...
for i in range(1, 34):  # 指明33个location
    ...
    for j in range(0, 24):  # 指明周一到周日
        ...
        zhongwei_registered = np.zeros((6, 7), dtype=float)
        zhongwei_casual = np.zeros((6, 7), dtype=float)
        ...
        for k in range(1, 7):  # 指明周数
            ...
            location_hour_week = location_hour_week.sort_values(by=['hour'])  # 按照时间顺序从小到大排列
            # print(location_weekday_week.head(3))
            # print(len(location_weekday_week['hour']))
            registered = location_hour_week['registered'].values
            casual = location_hour_week['casual'].values
            print(registered)
            print(casual)
            # print(np.median(temp))
            if np.sum(registered) != 0:
                for m in range(7):  # 记录每小时的中位数占比
                    zhongwei_registered[k-1][m] = (location_hour_week.iloc[m, 2])/np.sum(registered)  # 记录每小时的中位数占比
            ...
            if np.sum(casual) != 0:
                for m in range(7):  # 记录每小时的中位数占比
                    ...
                    zhongwei_casual[k-1][m] = (location_hour_week.iloc[m, 3]) / np.sum(casual)  # 记录每小时的中位数占比
            else:
                for n in range(7):  # 记录每小时的中位数占比
                    # zhongwei_registered[k - 1][n] = 0  # 记录每小时的中位数占比
                    zhongwei_casual[k - 1][n] = 0  # 记录每小时的中位数占比
...
result_location_weekday['flow2'] = result_location_weekday['registered'].map(float) + result_location_weekday['casual'].map(float)
result_location_weekday.pop('registered')
result_location_weekday.pop('casual')
result_location_weekday.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/9_10规则二_6周_人群分类.csv', index=False)
