...
print('异常值处理完毕')
X_train_divided.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/xgboost/所有train进行异常值处理后的特征集.csv', index=False)
'''
'''
# 以复制形式扩充训练集
...
train = pd.read_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/xgboost/所有train进行异常值处理后的特征集.csv')


# 分割食堂和平常
X_train_divided_shitang = train.drop(train[(train['big_loc'] != 'shitang')].index)
X_train_divided_else = train.drop(train[(train['big_loc'] == 'shitang')].index)
X_test_divided_shitang0 = test.drop(test[(test['big_loc'] != 'shitang')].index)
X_test_divided_else0 = test.drop(test[(test['big_loc'] == 'shitang')].index)


print(X_train_divided_shitang.head(3))
# 正常处理开始，分割训练集测试集
...
features_shitang = ['month', 'week', 'flow', 'flow2', 'flow3', 'hour', 'max_temp', 'min_temp', 'location_8', 'location_10', 'location_12',
                    'location_29',
                    ...
                    'hour_cat_20', 'hour_cat_21', 'hour_cat_22', 'hour_cat_23', 'hour_cat_6', 'hour_cat_7',
                    'hour_cat_8', 'hour_cat_9', 'weather1_1',
...
features_else = ['month', 'week', 'flow', 'flow2', 'flow3', 'hour', 'max_temp', 'min_temp', 'location_1', 'location_11', 'location_13',
                 'location_14',
                 ...
                 'weather1_1', 'weather1_4', 'weather2_1', 'weather2_2', 'working_no', 'working_yes']
...
print('其他地点测试集一共', len(y_pred_else), '条数据')
zero_count = 0
...
temp_result['month'] = temp_result['month'].astype(int)
temp_result['day'] = temp_result['day'].astype(int)
temp_result['hour'] = temp_result['hour'].astype(int)
temp_result['location'] = temp_result['location'].astype(int)
temp_result = temp_result.sort_values(by=['month', 'day', 'hour', 'location'])  # 进行排序
print(temp_result.head(3))
y_pred = temp_result['pred_flow']  # 抽取全部预测流量

y_pred.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/xgboost/最后的最后XGBoost使用八周数据_去除异常.csv', index=False)
'''
...
def GBM_registered(argsDict):
    ...
    # gbm_registered.fit(X_train_divided[features_shitang], y_train_divided_registered)
    # y_pred_registered_para = gbm_registered.predict(X_test_divided[features_shitang])
...
best_registered = fmin(GBM_registered, space_registered, algo=algo_registered, max_evals=50)
...
def GBM_casual(argsDict):
    ...
    # gbm_casual.fit(X_train_divided[features_else], y_train_divided_casual)
    # y_pred_casual_para = gbm_casual.predict(X_test_divided[features_else])
...
best_casual = fmin(GBM_casual, space_casual, algo=algo_casual, max_evals=50)
print(best_casual)
