...
for i in range(1, 34):  # 指明33个location
    ...
    for j in range(1, 8):  # 指明周一到周日
        ...
        zhongwei = np.zeros((8, 24), dtype=float)
        ...
        for k in range(1, 9):  # 指明周数
            ...
            if np.sum(temp) != 0:  # 到底是mean好一些还是sum好一些？
                for m in range(24):  # 记录每小时的中位数占比
                    zhongwei[k-1][m] = (location_weekday_week.iloc[m, 2])/np.sum(temp)  # 记录每小时的中位数占比
...
result_location_weekday.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/10_11规则一_8周.csv', index=False)
