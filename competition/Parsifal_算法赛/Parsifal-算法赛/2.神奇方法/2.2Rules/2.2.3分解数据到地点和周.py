        ...
        for k in range(1, 7):  # 指明周数
            ...
            if np.mean(temp) != 0:
                for m in range(24):  # 记录每小时的中位数占比
                    zhongwei[k-1][m] = (location_weekday_week.iloc[m, 2])/np.mean(temp)  # 记录每小时的中位数占比
...
result_location_weekday.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/9_10规则一_6周.csv', index=False)
