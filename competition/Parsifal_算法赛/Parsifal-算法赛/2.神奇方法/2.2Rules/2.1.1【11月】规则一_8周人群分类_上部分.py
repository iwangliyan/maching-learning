...
for i in range(1, 34):  # 指明33个location
    ...
    for j in range(1, 8):  # 指明周一到周日
        ...
        zhongwei_registered = np.zeros((8, 24), dtype=float)
        zhongwei_casual = np.zeros((8, 24), dtype=float)
        ...
        for k in range(1, 9):  # 指明周数
            ...
            if np.sum(registered) != 0:
                for m in range(24):  # 记录每小时的中位数占比
                    ...
                    # zhongwei_casual[k-3][m] = (location_weekday_week.iloc[m, 3]) / np.sum(casual)  # 记录每小时的中位数占比
            else:
                for n in range(24):  # 记录每小时的中位数占比
                    zhongwei_registered[k-1][n] = 0  # 记录每小时的中位数占比
                    # zhongwei_casual[k - 3][n] = 0  # 记录每小时的中位数占比

            if np.sum(casual) != 0:
                for m in range(24):  # 记录每小时的中位数占比
                    # zhongwei_registered[k - 3][m] = (location_weekday_week.iloc[m, 2]) / np.sum(registered)  # 记录每小时的中位数占比
                    zhongwei_casual[k-1][m] = (location_weekday_week.iloc[m, 3]) / np.sum(casual)  # 记录每小时的中位数占比
            else:
                for n in range(24):  # 记录每小时的中位数占比
                    # zhongwei_registered[k - 3][n] = 0  # 记录每小时的中位数占比
...
result_location_weekday.to_csv('E:/学习/研究生1/创新竞赛/2018年算法赛/data/rules/10_11规则一_8周_人群分类.csv', index=False)
